/**
 * @file Lab4Page.js
 * @description Сторінка лабораторної роботи №4 — сортування злиттям (Merge Sort).
 *
 * Функціонал:
 * - Теоретичний блок (опис, складність, псевдокод)
 * - Інтерактивний блок (введення масиву, кнопки, повзунок швидкості)
 * - Візуалізація стовпчиків (синій для порівняння, блакитний для завершення)
 * - Аналітика (порівняння, перестановки, час — динамічно)
 * - Звукові ефекти (тик на крок, мелодія при завершенні)
 *
 * Логіка:
 * - mergeSort() повертає { steps, comparisons, swaps, executionTimeMs }
 * - steps — масив кроків для покрокової/авто візуалізації
 * - Кожен крок: { array, h, step, i, j, k, action, comparisons, swaps }
 */

import { mergeSort } from '../../core/algorithms/mergeSort.js';
...
const PSEUDOCODE = `void Sort_Merge(int *a, int n) {
    int mid = n / 2;
    if (n % 2 == 1) mid++;
...
export const Lab4Page = {
  ...
  mount(container) {
    ...
    const html = `
      <div class="p-6 lg:p-8 max-w-6xl mx-auto space-y-8">
        ...
        <section class="prose dark:prose-invert max-w-none animate-fade-in-up" style="animation-delay: 0.1s; animation-fill-mode: both;">
          ...
          <p>Сортування злиттям — ефективний алгоритм, який використовує підхід "розділяй і володарюй". Алгоритм розділяє масив на дві частини, рекурсивно сортує їх, а потім зливає відсортовані частини. У цій реалізації використовується ітеративний (bottom-up) підхід.</p>
          <p><strong>Складність:</strong> O(n log n) у всіх випадках (середній, найгірший, найкращий).</p>
     ...
     * state: { array, h, step, i, j, k, action, comparisons, swaps }
    ...
    function renderBars(arr, state = {}, playSound = false) {
      ...
      viz.innerHTML = arr.map((val, idx) => {
        ...
        else if (idx === state.i || idx === state.j) classes += ' bg-blue-500 dark:bg-blue-600 ring-2 ring-offset-2 ring-blue-400 shadow-lg shadow-blue-500/30';
    ...
    document.getElementById('btn-sort').addEventListener('click', () => {
      ...
      showError('');
      const result = mergeSort(parsed.data);
      lastResult = result;
      steps = result.steps;
      stepIndex = 0;
      updateStats(result);
