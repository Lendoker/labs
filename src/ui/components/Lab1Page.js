/**
 * @file Lab1Page.js
 * @description Сторінка лабораторної роботи №1 — сортування вибором.
 *
 * Функціонал:
 * - Теоретичний блок (опис, складність, псевдокод)
 * - Інтерактивний блок (введення масиву, кнопки, повзунок швидкості)
 * - Візуалізація стовпчиків (зелений/жовтий/синій/блакитний)
 * - Аналітика (порівняння, перестановки, час — динамічно)
 * - Звукові ефекти (тик на крок, мелодія при завершенні)
 *
 * Логіка:
 * - selectionSort() повертає { steps, comparisons, swaps, executionTimeMs }
 * - steps — масив кроків для покрокової/авто візуалізації
 * - Кожен крок: { array, sortedUpTo, currentMin, comparingWith, action, comparisons, swaps }
 */
...
export const Lab1Page = {
  ...
  mount(container) {
    ...
    const html = `
      <div class="p-6 lg:p-8 max-w-6xl mx-auto space-y-8">
        ...
        <section class="prose dark:prose-invert max-w-none animate-fade-in-up" style="animation-delay: 0.1s; animation-fill-mode: both;">
          ...
          <p>Сортування вибором — простий алгоритм. На кожній ітерації знаходимо мінімальний елемент серед невідсортованих і ставимо його на поточну позицію.</p>
     ...
     * state: { array, sortedUpTo, currentMin, comparingWith, action, comparisons, swaps }
     * Кольори: зелений (відсортовано), жовтий (мінімум), синій (порівняння), блакитний (завершено).
    ...
    function renderBars(arr, state = {}, playSound = false) {
      ...
      viz.innerHTML = arr.map((val, i) => {
        ...
        else if (i === state.currentMin) classes += ' bg-amber-500 dark:bg-amber-600 shadow-lg shadow-amber-500/40 ring-2 ring-amber-400 scale-105';
    ...
    document.getElementById('btn-sort').addEventListener('click', () => {
      ...
      showError('');
      const result = selectionSort(parsed.data);
      lastResult = result;
      steps = result.steps;
      stepIndex = 0;
      updateStats(result);
