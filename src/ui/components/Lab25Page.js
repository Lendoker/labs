/**
 * @file Lab25Page.js
 * @description Лабораторна №25 — Мінімальний кістяк, алгоритм Краскала.
 */
...
export const Lab25Page = {
  mount(container) {
    let matrix = parseMatrixText(EXAMPLE_MATRIX);
    let trace = kruskalMst(undirectedEdgesFromMatrix(matrix), matrix.length);
    let logLines = ['[menu] Матриця ваг → Краскал'];

    container.innerHTML = `
      <div class="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-8">
        <header><h1 class="text-2xl font-bold">Лабораторна робота №25 — Алгоритм Краскала</h1></header>
        <section class="prose dark:prose-invert max-w-none">
          <h2>Теоретичний блок</h2>
          <p><strong>Мінімальне кістякове дерево (MST)</strong> — підмножина ребер зв’язного зваженого графа, що з’єднує всі вершини без циклів із мінімальною сумою ваг.</p>
          <p><strong>Краскал:</strong> сортуємо ребра за вагою; додаємо ребро, якщо воно не з’єднує вершини, що вже в одній компоненті (Union-Find). Зупиняємось після n−1 ребер.</p>
          <p>Складність O(m log m). Зручний для розріджених графів.</p>
        </section>
    ...
    function recompute() {
      trace = kruskalMst(undirectedEdgesFromMatrix(matrix), matrix.length);
      playback.setStep(0);
      playback.render();
    }
    ...
    const playback = wireStepPlayback({
      ...
      onStep: renderStep,
      onRun: () => pushLog(`MST вага=${trace.totalWeight}`),
    ...
    recompute();
    pushLog('Демо: граф з методички (5 вершин)');
  },
};
